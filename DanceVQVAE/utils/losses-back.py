...
#     def __init__(self,args, recons_loss, nb_joints, w_motion=1, w_joints=0.05, w_vel=0.06, w_acc=0.06, w_foot=15):
...
class ReConsLoss(nn.Module):
    ...
    def forward_all(self, motion_pred, motion_gt):
        ...
        foot_mask[[idx - 1 for idx in foot_idx]] = True
        foot_mask = foot_mask.repeat(3)
        pred_foot_joints = pred_joints[..., foot_mask].view(-1, motion_pred.shape[1], len(foot_idx), 3)  # [batch_size, seq_len, 4, 3]
        pred_foot_vel = pred_foot_joints[:, 1:] - pred_foot_joints[:, :-1]  # [batch_size, seq_len-1, 4, 3]
        pred_foot_h = pred_foot_joints[:, :-1, :, 1]  # [batch_size, seq_len-1, 4]
        contact = self.foot_detect(pred_foot_vel, pred_foot_h)
        foot_loss = self.Loss(pred_foot_vel * contact.unsqueeze(-1), torch.zeros_like(pred_foot_vel)) * contact.sum() / (contact.sum() + 1e-7)
...
if __name__ == "__main__":
    ...
    def mix_masked_mse(prediction, target, mask, batch_mask, contact_mask=None):
        ...
        loss = Loss(prediction, target).mean(dim=-1, keepdim=True)  # [b,t,4,1]
    ...
    print('contact', contact.shape)     # [batch_size, seq_len-1, 4]
    B,T,J,C = pred_foot_vel.shape
    ...
    Loss(pred_foot_vel * contact.unsqueeze(-1), torch.zeros_like(pred_foot_vel)) * contact.sum() /(contact.sum() + 1e-7)
    
    print("Total Loss:", total_loss.item())
    print("Motion Loss:", motion_loss.item())
    print("XZ Velocity Loss:", xz_vel_loss.item())
    print("XZ Acceleration Loss:", xz_acc_loss.item())
    print("Rotation Loss:", rot_loss.item())
    print("Rotation Acceleration Loss:", rot_acc_loss.item())
    print("Foot Loss:", foot_loss.item())