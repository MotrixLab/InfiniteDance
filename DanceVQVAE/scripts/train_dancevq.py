...
import DanceVQVAE.utils.losses as losses
import DanceVQVAE.utils.utils_model as utils_model
from DanceVQVAE.dataset import dataset_VQ
...
def getfid(checkpoint_path):
    result = subprocess.run(
        [
            "python", "DanceVQVAE/scripts/Inferrence_Reconstruct_momask_all.py",
...
if args.vel_decoder:
    args.out_dir = 'DanceVQVAE/output/vel_decoder'
elif args.rot_decoder:
    args.out_dir = 'DanceVQVAE/output/rot_decoder'

    

   
current_time = (datetime.now() ).strftime(f"25%m%d_%H%M")

#naming expname accroding to date
args.out_dir = os.path.join(args.out_dir, f'exp_test_{current_time}')
...
eval_dataset = dataset_VQ.DATALoader_val(args.dataname,
                                                   args.batch_size,
                                                   sfile=args.split_file_eval,
                                                   window_size=args.window_size,
                                                   unit_length=2**args.down_t)
...
for nb_iter in range(1, args.total_iter + 1):
    ...
    pred_motion, pred_motion_xz, pred_motion_rot, loss_commit, perplexity, utilization_stats = net(gt_motion)
    # whether to train additional encoder
    if args.vel_decoder:
        pred_motion[:, :, 3:5] = pred_motion_xz
    if args.rot_decoder:
        pred_motion[:, :, [1, 2, *range(68, 194)]] = pred_motion_rot
