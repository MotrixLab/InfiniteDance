...
def euclidean_distance_matrix_torch(matrix1, matrix2):
    ...
    assert matrix1.shape[1] == matrix2.shape[1]
    d1 = -2 * torch.matmul(matrix1, matrix2.T)  # (num_test, num_train)
    d2 = torch.sum(matrix1 ** 2, dim=1, keepdim=True)  # (num_test, 1)
    d3 = torch.sum(matrix2 ** 2, dim=1).unsqueeze(0)   # (1, num_train)
...
def calculate_top_k(mat, top_k):
    ...
    gt_mat = np.zeros_like(mat) # np.expand_dims(np.arange(size), 1).repeat(size, 1)
