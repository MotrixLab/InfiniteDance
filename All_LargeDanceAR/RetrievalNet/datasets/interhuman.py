...
class InterHumanDataset(data.Dataset):
    def __init__(self, opt):
        ...
        ignore_list = []
        try:
            ignore_list = open(os.path.join(opt.splitdir, "ignore_list.txt"), "r").readlines()
        except Exception as e:
            print(e)
        data_list = []
        if self.opt.MODE == "train":
            try:
                data_list = open(os.path.join(opt.splitdir, "train.txt"), "r").readlines()
            except Exception as e:
                print(e)
        elif self.opt.MODE == "val":
            try:
                data_list = open(os.path.join(opt.splitdir, "val.txt"), "r").readlines()
            except Exception as e:
                print(e)
        elif self.opt.MODE == "test":
            try:
                data_list = open(os.path.join(opt.splitdir, "test.txt"), "r").readlines()
