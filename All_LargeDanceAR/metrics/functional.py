...
def write2pkl(dances, dance_names, config, expdir, epoch, rotmat):
    ...
    for i in tqdm(range(len(dances)),desc='Generating Jsons'):
        ...
        dance_path = os.path.join(ep_path, dance_names[i] + '.npy')
        # if not os.path.exists(dance_path):
        #     os.makedirs(dance_path)

        # with open(dance_path, 'w') as f:
        np.save(dance_path, npy_data)
        print(f"saved to {dance_path}")
...
def visualizeAndWrite(results,config,expdir,dance_names, epoch, quants=None):
    print(dance_names)
    # dance_names = [name.replace('.json', '') for name in dance_names]

    if config.rotmat:
        smpl = SMPL(model_path=config.smpl_dir, gender='NEUTRAL', batch_size=1)
    np_dances = []
    np_dances_original = []
    dance_datas = []
...
def load_data_all(data_dir, music_dir,train_file,interval=120, move=40, rotmat=False, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    tot = 0
    ...
    for fname in fnames:
        ...
        np_dance = np.array(sample_dict.reshape((-1,24*3))) #
...
def load_data_aist(data_dir, interval=120, move=40, rotmat=False, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    ...
    if ".ipynb_checkpoints" in fnames:
        fnames.remove(".ipynb_checkpoints")
    for fname in tqdm(fnames, desc="Loading JSON files"):
        path = os.path.join(data_dir, fname)
        with open(path) as f:
            # print(path)
            sample_dict = json.loads(f.read())
    ...
    if music_normalize:
        print("begin calculate music_np")
        music_mean_path = "/data1/hzy/HumanMotion/Bailando/data/music_mean.npy"
        music_std_path = "/data1/hzy/HumanMotion/Bailando/data/music_std.npy"

        if not (os.path.exists(music_mean_path) and os.path.exists(music_std_path)):
            music_np = np.stack(music_data).reshape(-1, music_data[0].shape[1])
            music_mean = music_np.mean(0)
            music_std = music_np.std(0)
            music_std[(np.abs(music_mean) < 1e-5) & (np.abs(music_std) < 1e-5)] = 1
            ...
            print(music_np)
            np.save(music_mean_path, music_mean)
            np.save(music_std_path, music_std)
        else:
            print("Music mean and std files already exist, skipping calculation.")
        print('calculating norm mean and std')
        music_data_norm = [ (music_sub_seq - music_mean) / (music_std + 1e-10) for music_sub_seq in music_data ]
        with open('/mnt/lustressd/lisiyao1/dance_experiements/music_norm.json', 'w') as fff:
            sample_dict = {
...
def load_test_data_aist(data_dir, rotmat, move, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
    ...
    #print(fnames)
    # fnames = fnames[:60]  # For debug
    for fname in tqdm(fnames, desc="Processing Files"):
        path = os.path.join(data_dir, fname)
        print(path)
        with open(path) as f:
...
def load_test_data_all(test_file,data_dir, rotmat, move, external_wav=None, external_wav_rate=1, music_normalize=False, wav_padding=0):
